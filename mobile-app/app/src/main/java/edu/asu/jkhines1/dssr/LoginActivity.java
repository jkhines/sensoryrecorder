...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
     ...
     */
    private UserLoginTask mAsyncTask = null;
    ...
    private void setupActionBar() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            // Show the Up deviceApprovalImageButton in the action bar.
    ...
    private void attemptLogin() {
        ...
        if (cancel) {
            ...
            showProgress(true);
            mAsyncTask = new UserLoginTask(email, password);
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
        ...
        UserLoginTask(String email, char[] password) {
            mEmail = email;
            mPassword = password;
        }
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            try {
                Tokens tokens =
                        TokenHelper.resourceOwnerPasswordFlow(getApplicationContext(),
                                ApplicationConfig.AUTH_SERVER_URL,
                                ApplicationConfig.CLIENT_ID, ApplicationConfig.CLIENT_SECRET,
                                mEmail, mPassword);
            } catch (Exception e) {
                return false;
            } finally {
                for (int i = 0; i != mPassword.length; ++i) { // Clear password.
                    mPassword[i] = '\u0000';
                }
            }

            // Change to the data summary screen.
