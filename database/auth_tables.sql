--
-- DB structures for OIDC Server functionality, PostgreSQL
--
...
CREATE OR REPLACE FUNCTION create_user_authority()
RETURNS TRIGGER AS
$$
BEGIN
    INSERT INTO user_info(sub, preferred_username, name, email, email_verified)
    VALUES(gen_random_uuid(), NEW.username, NEW.username, NEW.username, false);
...
CREATE TRIGGER user_added
  ...
  ON users
  FOR EACH ROW
  EXECUTE PROCEDURE create_user_authority();
...
ALTER TABLE ONLY client_grant_type
    ADD CONSTRAINT client_grant_type_pkey PRIMARY KEY (owner_id, grant_type);
...
ALTER TABLE ONLY client_redirect_uri
    ADD CONSTRAINT client_redirect_uri_pkey PRIMARY KEY (owner_id, redirect_uri);
...
ALTER TABLE ONLY client_scope
    ADD CONSTRAINT client_scope_pkey PRIMARY KEY (owner_id, scope);
...
CREATE TABLE IF NOT EXISTS whitelisted_site_scope (
    owner_id BIGINT,
    scope VARCHAR(256),
      CONSTRAINT whitelisted_site_scope_unique UNIQUE (owner_id, scope)
);
